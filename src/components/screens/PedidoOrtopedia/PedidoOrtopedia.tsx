...
export const PedidoOrtopedia: React.FC = () => {
  ...
  const obtenerPedidos = async () => {
    try {
      const response = await axios.get('http://vps-5301866-x.dattaweb.com:9000/api/pedidos/ortopedia');
  ...
  const eliminarPedido = async (id: number) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://vps-5301866-x.dattaweb.com:9000/api/pedidos/ortopedia/${id}`);
